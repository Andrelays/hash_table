...
<img src="https://github.com/Andrelays/hash_table/blob/main/pictures/perf_1.png?raw=true" width = 100%>
...
Первое контрольное время работы программы: $5.97 \cdot 10^8$ тиков.
...
Новое время стресс теста - $4.97 \cdot 10^8$ тиков. Встраивание дало прирост 20% в скорости работы программы, так как были убраны множественные вызовы функций.
...
Вторая версия использовала встроенные функции SSE `_mm_crc32_u8 (crc, char)`, $3.64 \cdot 10^8$ тиков:
...
В моих данных самое длинное слово размером 16 букв, поэтому я использовал 16 байтные вектора `__m128`. Итак, идея проста. Давайте выделим 16 байт для каждого слова. Таким образом, сложность алгоритма для сравнения двух слов равна O(1).
...
Новое время - $3.13 \cdot 10^8$ тиков (прирост 13%)

Итоговое время стресс теста - $3.13 \cdot 10^8$ тиков (прирост 19 %)
...
$Coeff = \frac{boost}{amount \space asm \space strings} \cdot 100 = \frac{190}{12} = 15.8$

*Таблица результатов*:

|Оптимизация               |Тики ($10^8$)      |Ускорение(в сравнении с началом) |
|:-------------------------|:-----------------:|:-------------------------------:|
|Начало с `-O3`            | 5.97              |1.00x                            |
|Встройка                  | 4.97              |1.20x                            |
|Выравнивание              | 4.38              |1.36x                            |
|Векторизация CRC32        | 3.64              |1.64x                            |
|Векторизация `strcmp()`   | 3.13              |1.90x                            |
